...
# topic_perc = pd.read_csv("hhi_ie.csv", usecols = topic_names, dtype = col_dict, engine = "python")
#
...
#
# # Write dictionary to csv file
# with open('mahal_first_ie.csv', 'w', newline = '') as csv_file:
#     fields = ["cluster_id", "id", "seq", "Topic1", "Topic2"]
#     fields += ["T" + str(i) for i in range(1, 20)]
...
# mina_dict = load_obj("mina_dict_ie_results")
# sentiment_scores = "sentiment_scores_nrc.csv"
# sentiment = pd.read_csv(sentiment_scores, usecols = ["ID", "anger", "anticipation", "disgust", "fear", "joy", "negative",
#                                                      "positive", "sadness", "surprise", "trust", "sentiment"])
#
# for i in range(sentiment.shape[0]):
#     print(i)
#     article_id = sentiment.loc[i, "ID"]
#     for key, value in mina_dict.items():
#         for dict in value:
#             if (article_id in dict):
#                 print("GOT HERE")
#                 topic_list = dict[article_id]
...
#                 dict[article_id] = topic_list

#print("DONE WITH DICTIONARY")
# # Save dictionary object
#save_obj(mina_dict, "mina_dict_sent")
# mina_dict_sent = load_obj("mina_dict_sent")
#
# with open('sentiment_nrc_ie.csv', 'w', newline = '') as csv_file:
#     fields = ["cluster_id", "id", "seq", "Topic1", "Topic2"]
#     fields += ["T" + str(i) for i in range(1, 20)]
...
#     writer = csv.DictWriter(csv_file, fields)
#     writer.writeheader()
#     for key, value in mina_dict_sent.items():
#         for dict in value:
#             for article_id, article_value in dict.items():
#                 topic_names = fields
#                 topic_values = [key, article_id, article_value[-12], article_value[0],
...
# sentiment_file = "sentiment_nrc_ie.csv"
# col_names = ["cluster_id", "id", "seq", "Topic1", "Topic2", "anger", "anticipation", "disgust", "fear", "joy", "negative", "positive", "sadness", "surprise", "trust", "sentiment"]
# sentiment_values = [int, int, np.float64, str, str] + [np.float64 for i in range(1, 12)]
...
#
# word_count = pd.read_csv("../ie_word_count.csv", usecols = ["ID", "WordCount"], dtype = {"ID": int, "WordCount": int})
# #print(word_count.head())
# word_count_dict = {k : v for k, v in zip(word_count["ID"], word_count["WordCount"])}
#
# for i in range(sentiments.shape[0]):
#     print(i)
#     article_id = sentiments.loc[i, "id"]
#     if (article_id in word_count_dict):
#         word_count = word_count_dict[article_id]
...
#         sentiment_list.append(seq)
#         id_dict[id] = sentiment_list
#         sentiment_dict[cluster_id] = [id_dict]
#
...
#                  dict[key] = sent_list
#
# save_obj(sentiment_dict, "mina_sent_mahalanobis_first")

# Write to csv file
mina_dict = load_obj("mina_sent_mahalanobis_first")

with open('sentiment_mahalanobis.csv', 'w', newline = '') as csv_file:
    fields = ["cluster_id", "id", "seq", "Topic1", "Topic2"]
